...
import { aggregateSearchResults } from '../utils/aggregator';
...
router.get('/', async (c) => {
  try {
    ...
    const limit = params.limit ? parseInt(params.limit) : config.search.defaultLimit;
    const validLimit = Math.max(1, limit); // Ensure positive number
    
    // If user provided pageno, use original single-page logic (backward compatibility)
    if (params.pageno) {
      logger.info(`User provided pageno=${params.pageno}, using single-page mode`);
    ...
    // Use new aggregation logic for limit-based requests
    logger.info(`Using aggregation mode with limit=${validLimit}`);
    
    // Create base parameters (without pageno)
    const baseParams = { ...params };
    delete baseParams.pageno; // Remove pageno to let aggregator handle it
    delete baseParams.limit;  // Remove limit from forwarded params
    
    // Use aggregation
    const aggregatedResult = await aggregateSearchResults(
      config.endpoints.search.urls,
      baseParams,
      validLimit
    );
    
    logger.info(`Aggregation completed, returning ${aggregatedResult.results?.length || 0} results`);
    return c.json(aggregatedResult);
  } catch (error) {
